...
{
    //const struct timespec sleep_time_struct = {0, (long)(sleep_time*1e6)};
...
{
    if ((n < 1) || (n > 26))
	error("int_to_letter(n), 1<=n<=26, you provided n=%d", n);
    char c = n+96;
    return(c);
}
...
void daemonize(char *stdio_file_name)
{
    ...
    f = freopen(stdiopath, "w", stdout);
    sprintf(stdiopath, "%s.%s", SC_STDERR, stdio_file_name);
    f = freopen(stdiopath, "a", stderr);
